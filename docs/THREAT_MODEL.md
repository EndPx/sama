...
React inspection tools are development-only, served from pinned local packages, and return no scripts in production. The overlays are off by default, including during sensitive reveal work; a developer may opt in locally with `NEXT_PUBLIC_ENABLE_REACT_DEVTOOLS=1` only when no real reveal material is present. The same-origin tool route accepts a fixed allowlist of two package assets, not arbitrary paths. The component showcase is also unavailable in production.
