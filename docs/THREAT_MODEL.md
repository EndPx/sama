...
- The new demo entry uses Privy email OTP, Google or Apple OAuth, or wallet sign in. OTP input stays in client component state and is sent only through Privy's SDK; the app does not store or log it. OAuth redirects and method availability depend on Privy and its dashboard configuration. The public App ID is browser configuration; the Privy app secret remains server-only and is not used by this client.
- The signed-in workspace is a client-side presentation gate, not private data access control. Public educational and legacy transaction URLs, blockchain reads, and contract calls are not protected by the page gate. Contract eligibility, wallet ownership, chain checks, and receipt confirmation remain the security boundaries. A missing public App ID fails closed on the demo route outside local Anvil mode rather than silently treating a browser wallet as a Privy session.
...
- A corrupt pre-commit draft can be restored through a local file picker without developer tools. The importer limits file size and validates schema plus chain, offering, and wallet before replacing storage; after commitment, it also checks the onchain commitment. The file remains readable to the browser and must be kept private.
