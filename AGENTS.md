...
2. Read `docs/THREAT_MODEL.md`; completion means the change names any trust-boundary or attack-surface impact.
3. Select the next incomplete milestone in `docs/IMPLEMENTATION_PLAN.md`; completion means its exit gate passes before later milestones begin.
4. Run the relevant build, tests, static analysis, and secret scan before handoff; completion means results are reported with commands and no known failure is hidden.
