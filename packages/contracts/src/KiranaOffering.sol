...
contract KiranaOffering is AccessControl, Pausable, ReentrancyGuard {
    ...
    function phase() public view returns (Phase) {
        ...
        if (hasRole(CANCELLER_ROLE, address(0))) return Phase.CANCELLED; // unreachable sentinel; cancellation is below
    ...
    function revealBid(uint128 amount, uint64 maxFdv, bytes32 nonce) external whenNotPaused {
        if (currentPhase() != Phase.REVEAL) revert WrongPhase();
    ...
    function settle(address[] calldata ordered) external whenNotPaused {
        ...
        for (uint256 i; i < ordered.length; ++i) {
            Bid storage b = bids[ordered[i]];
            if (b.maxFdv > candidate) above += b.amount;
            else if (b.maxFdv == candidate) atTier += b.amount;
        }
        uint256 remaining = capacity - above;
        for (uint256 i; i < ordered.length; ++i) {
            Bid storage b = bids[ordered[i]];
            uint256 accepted =
                b.maxFdv > candidate ? b.amount : (b.maxFdv == candidate ? uint256(b.amount) * remaining / atTier : 0);
            _setBidAccounting(ordered[i], accepted);
        }
        finalized = true;
        successful = true;
        clearingFdv = candidate;
        acceptedTotal = capacity;
        issuerProceeds = capacity;
        emit Settled(true, candidate, capacity);
    }
    ...
    function refundable(address bidder) public view returns (uint256) {
        ...
        if (!b.revealed) return b.amount;
        if (!successful) return b.amount;
        return b.amount - acceptedOf(bidder);
    }

    function acceptedOf(address bidder) public view returns (uint256) {
        Bid storage b = bids[bidder];
        if (!successful || !b.revealed) return 0;
        if (b.maxFdv > clearingFdv) return b.amount;
        if (b.maxFdv < clearingFdv) return 0;
        uint256 atTier;
        for (uint256 i; i < revealedBidders.length; ++i) {
            if (bids[revealedBidders[i]].maxFdv == clearingFdv) atTier += bids[revealedBidders[i]].amount;
        }
        uint256 above;
        for (uint256 i; i < revealedBidders.length; ++i) {
            if (bids[revealedBidders[i]].maxFdv > clearingFdv) above += bids[revealedBidders[i]].amount;
        }
        return uint256(b.amount) * (acceptedTotal - above) / atTier;
    }
